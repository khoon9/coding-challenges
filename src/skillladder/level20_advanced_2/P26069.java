...
public class P26069 {
    // 상태가 전이된다. 시간 순서대로 사건이 주어진다고할 때 상태가 전이된 대상의 개수를 구한다.
    // ChongChong 만이 초기에 전이가 가능한 상태를 가진다.
    public static void main(String[] args){
        try{
            ...
            List<String[]> targetCases = new ArrayList<>();
            Set<String> targetSet = new HashSet<>();
            targetSet.add("ChongChong");
            for (int i=0; i<N; i++){
            ...
            for (String[] targetCase: targetCases){
                if (targetSet.contains(targetCase[0])){
                    targetSet.add(targetCase[1]);
                }
                if (targetSet.contains(targetCase[1])){
                    targetSet.add(targetCase[0]);
                }
            }
