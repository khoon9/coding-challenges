            ...
            for (int i=0; i<N; i++){
                ...
                if (line.length()>=M){
                    if (targetMap.containsKey(line)){
                        targetMap.put(line,targetMap.get(line)+1);
                    } else {
                        targetMap.put(line,1);
                    }
                }
            }
            targetMap.entrySet().stream()
                    .sorted(Comparator
                            .comparing((Map.Entry<String, Integer> target)->target.getValue(), Comparator.reverseOrder())
                            .thenComparing(target->target.getKey().length(), Comparator.reverseOrder())
                            .thenComparing(target->target.getKey())
                    ).collect(Collectors.toList())
                            .stream().forEach(target->sb.append(target.getKey()).append("\n"));
