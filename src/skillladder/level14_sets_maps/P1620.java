...
public class P1620 {
    // N 개의 name 이 들어오면, 들어오는 순서 order를 함께 저장
    // 이후 M 개의 입력에서
    // 숫자가 들어온다면 해당하는 name을
    // 문자열이 들어온다면 해당하는 order를 반환
    public static void main(String[] args){
        try{
            ...
            for (int i=0; i<N; i++){
                targetMap.put(br.readLine(),i+1);
            }
            Map<Integer,String> targetMapReverse = targetMap.entrySet().stream()
                    .collect(Collectors.toMap(entry->entry.getValue(),entry->entry.getKey()));
            for (int i=0; i<M; i++){
                String line = br.readLine();
                if (line.charAt(0)>='A' && line.charAt(0)<='Z'){
                    // 용문자열이 입력으로 들어옴
                    sb.append(targetMap.get(line)+"\n");
                } else {
                    // 숫자가 입력으로 들어옴
                    sb.append(targetMapReverse.get(Integer.parseInt(line))+"\n");
                }
            }
